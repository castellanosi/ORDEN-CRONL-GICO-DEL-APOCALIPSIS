...
import { 
  Sun, Moon, Menu, X, BookOpen, 
  Sparkles, Share2, ArrowUp, Clock
} from 'lucide-react';
...
11 Y vi un gran trono blanco y al que estaba sentado en él, de delante del cual huyeron la tierra y el cielo, y ningún lugar se encontró para ellos. 12 Y vi a los muertos, grandes y pequeños, de pie ante Dios; y los libros fueron abiertos, y otro libro fue abierto, el cual es el libro de la vida; y fueron juzgados los muertos por las cosas que estaban escritas en los libros, según sus obras. 13 Y al mar entregó los muertos que había en él; y la muerte y el Hades entregaron los muertos que había en ellos; y fueron juzgados cada uno según sus obras. 14 Y la muerte y el Hades fueron lanzados al lago de fuego. Esta es la muerte segunda. 15 Y el que no se halló inscrito en el libro de la vida fue lanzado al lago de fuego.
...
const MarkdownReader = () => {
  const [markdown] = useState(INITIAL_MARKDOWN);
  ...
  // Mejorar extracción de TOC para manejar Capítulo y Secciones
  const toc = useMemo(() => {
    ...
    return lines
      ...
      .map(line => {
        ...
        const text = line.replace(/^#+\s*/, '').replace(/<u>|<\/u>/g, '');
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Resume los puntos teológicos y cronológicos clave de este fragmento del Apocalipsis de forma estructurada:\n\n${markdown.substring(0, 5000)}`,
        config: {
          systemInstruction: "Eres un erudito bíblico. Proporciona resúmenes claros, profundos y profesionales en español.",
        }
      });
      setAiSummary(response.text || "No se pudo generar el resumen.");
    } catch (error) {
      console.error("Error al resumir:", error);
      setAiSummary("Error de conexión. Inténtalo de nuevo más tarde.");
    } finally {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'dark bg-black text-white' : 'bg-white text-black'}`}>
      ...
      <div className="fixed top-0 left-0 w-full h-1 z-[60] bg-gray-100 dark:bg-zinc-900">
        <div 
      ...
      {/* Header Noir */}
      <header className="fixed top-0 w-full bg-white/95 dark:bg-black/95 backdrop-blur-md border-b border-gray-100 dark:border-zinc-800 z-50 h-16">
        <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 font-black text-lg tracking-[0.2em]">
              <BookOpen size={22} />
              <span className="hidden sm:inline">APOCALIPSIS</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-black text-white dark:bg-white dark:text-black rounded-none text-xs font-bold tracking-widest hover:opacity-80 transition-all active:scale-95 disabled:opacity-50"
            >
              {isSummarizing ? (
              ...
              {isSummarizing ? 'SINTETIZANDO...' : 'SÍNTESIS IA'}
            </button>
            <button
              onClick={toggleTheme}
              className="p-2 transition-colors hover:text-gray-500"
            >
      ...
      <div className="max-w-7xl mx-auto flex">
        {/* Sidebar Noir */}
        <aside className={`
          fixed inset-y-0 left-0 w-72 bg-white dark:bg-black border-r border-gray-100 dark:border-zinc-800 transform transition-transform duration-300 z-50 lg:sticky lg:top-16 lg:h-[calc(100vh-64px)] lg:translate-x-0
          ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
        `}>
          <div className="p-8 h-full overflow-y-auto">
            <div className="flex items-center justify-between mb-10 lg:hidden">
              <span className="font-bold tracking-widest text-xs">ÍNDICE</span>
              <button onClick={() => setIsSidebarOpen(false)}><X size={20} /></button>
            </div>
            <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mb-8">Cronología</h3>
            <nav className="space-y-4">
              {toc.map((item, idx) => (
                <a
                  ...
                  href={`#${item.id}`}
                  className={`
                    block text-sm transition-all hover:translate-x-1
                    ${item.level === 1 ? 'font-black text-black dark:text-white uppercase tracking-wider' : 
                      item.level === 2 ? 'pl-4 font-bold text-gray-700 dark:text-zinc-300' : 'pl-8 text-gray-500 dark:text-zinc-500'}
                  `}
                >
                  {item.text}
                </a>
        ...
        {/* Contenido Principal */}
        <main className="flex-1 px-4 py-20 sm:px-8 lg:px-20 overflow-hidden min-h-screen">
          <article className="max-w-3xl mx-auto animate-fade-in">
            {aiSummary && (
              <div className="mb-16 p-10 bg-gray-50 dark:bg-zinc-900/50 border border-black/5 dark:border-white/10 rounded-none relative">
                <button 
                ...
                <div className="flex items-center gap-3 mb-6 font-black uppercase text-[10px] tracking-[0.4em]">
                  <Sparkles size={16} />
                  <span>SÍNTESIS TEOLÓGICA</span>
                </div>
                <div className="text-gray-800 dark:text-zinc-200 text-lg leading-relaxed italic border-l-4 border-black dark:border-white pl-8">
                  {aiSummary}
                </div>
              </div>
            )}

            <div className="prose prose-custom lg:prose-2xl dark:prose-invert max-w-none 
              prose-headings:font-black prose-headings:tracking-tighter prose-headings:text-black dark:prose-headings:text-white
              prose-p:text-gray-800 dark:prose-p:text-zinc-300 prose-p:leading-relaxed
              ...
              prose-blockquote:border-black dark:prose-blockquote:border-white prose-blockquote:bg-zinc-50 dark:prose-blockquote:bg-zinc-900/30 prose-blockquote:py-2 prose-blockquote:px-8
              ">
              <ReactMarkdown 
                remarkPlugins={[remarkGfm]}
                rehypePlugins={[rehypeRaw]}
                components={{
                  h1: ({node, ...props}) => <h1 id={props.children?.toString().toLowerCase().replace(/[^\w]+/g, '-')} className="mt-20 mb-10" {...props} />,
                  h2: ({node, ...props}) => <h2 id={props.children?.toString().toLowerCase().replace(/[^\w]+/g, '-')} className="mt-16 mb-8 border-b border-black/10 dark:border-white/10 pb-4" {...props} />,
                  h3: ({node, ...props}) => <h3 id={props.children?.toString().toLowerCase().replace(/[^\w]+/g, '-')} className="mt-12 mb-6" {...props} />,
                }}
              >
                {markdown}
              </ReactMarkdown>
            ...
            <div className="mt-32 pt-16 border-t border-black/5 dark:border-white/5 flex flex-col sm:flex-row items-center justify-between gap-8 text-[10px] font-black uppercase tracking-[0.4em] text-zinc-400">
              <div className="flex items-center gap-10">
                <span className="flex items-center gap-2"><Clock size={12} /> ESTUDIO CRONOLÓGICO</span>
                <span>RV 1960</span>
              </div>
              <button 
              ...
              >
                REGRESAR <ArrowUp size={12} className="inline ml-2" />
              </button>
            </div>
          </article>
        </main>
